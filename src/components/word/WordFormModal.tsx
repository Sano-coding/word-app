...
interface WordFormModalProps {
  ...
  /** overwriteId が渡された場合、そのレコードの意味・補足を上書き更新する（定着度・出題状況は保持） */
  onSubmit: (input: { word: string; meaning: string; note: string }, overwriteId?: string) => void
  onClose: () => void
}
...
export function WordFormModal({ mode, initialWord, onCheckDuplicate, onSubmit, onClose }: WordFormModalProps) {
  ...
  const [note, setNote] = useState(initialWord?.note ?? '')
  ...
  function handleSubmit(e: FormEvent) {
    ...
    onSubmit({ word: trimmedWord, meaning: meaning.trim(), note: note.trim() })
  }
    ...
    return (
      <Modal title="重複する単語があります" onClose={onClose}>
        <p className={styles.message}>
          「{pendingDuplicate.word}」は既に登録されています。意味・補足を上書きしますか？
          （定着度・出題状況は変更されません）
        </p>
  ...
  return (
    <Modal title={mode === 'create' ? '単語を登録' : '単語を編集'} onClose={onClose}>
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        <label className={styles.field}>
          <span className={styles.label}>補足（任意）</span>
          <textarea
            value={note}
